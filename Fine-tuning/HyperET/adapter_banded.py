...
class BandedLinear(nn.Module):
    def __init__(self, block_size, in_features, out_features, curvature: float = 0.01):
        super(BandedLinear, self).__init__()
        ...
        # 创建条纹矩阵的权重
        diags = 2 * block_size + 1
        offsets_positive = torch.arange(0, block_size + 1)
        offsets_negative = torch.arange(-1, -block_size - 1, -1)
        self.offsets = torch.cat([offsets_positive, offsets_negative])
        self.s_edge_index = torch.sparse.spdiags(torch.ones([diags, out_features]), self.offsets, (out_features, out_features)).coalesce().indices().cuda()
        initial_values = torch.zeros(self.s_edge_index.shape[1])  # 初始化为全0
        diag_mask = (self.s_edge_index[0] == self.s_edge_index[1])
        initial_values[diag_mask] = 1.0  # 将对角线元素初始化为1
        self.weights = nn.Parameter(initial_values, requires_grad=True)
    ...
    def forward(self, x, visual=False):
        ...
        banded_diagonal_weight = torch.sparse_coo_tensor(self.s_edge_index, self.weights, (self.out_features, self.out_features)).to_dense().to(orig_dtype)
        output_hyperbolic_filt_stretch = self.mobius_matvec(banded_diagonal_weight, fix_filt)
...
class Adapter_qv(nn.Module):
    def __init__(self, hidden_size, dim, curvature_ratio=0.01):
        super().__init__()
        self.adapter_attn_q = BandedLinear(block_size=dim, in_features=hidden_size, out_features=hidden_size, curvature=curvature_ratio)
        self.adapter_attn_v = BandedLinear(block_size=dim, in_features=hidden_size, out_features=hidden_size, curvature=curvature_ratio)
...
def set_Clip_Adapter(model, dim=8, s=0.1, curv=0.01):
    for _ in model.children():
        if type(_) == ResidualAttentionBlock:
            _.hyperbolic_attn = Adapter_qv(1024, dim=2, curvature_ratio=curv)
