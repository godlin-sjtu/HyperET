...
class DiagonalLinear(nn.Module):
    ...
    def pairwise_inner(self, x: torch.Tensor, y: torch.Tensor):
        ...
        x_time = torch.sqrt(1 / self.curvature + torch.sum(x2, dim=-1, keepdim=True))
        y_time = torch.sqrt(1 / self.curvature + torch.sum(y2, dim=-1, keepdim=True))
    ...
    def forward(self, x, visual=False):
        ...
        scaling_matrix = torch.diag(self.scaling).to(orig_dtype)
        output_hyperbolic_filt_stretch = self.mobius_matvec(scaling_matrix, fix_filt)
...
class Adapter_qv(nn.Module):
    def __init__(self, hidden_size, dim, curvature_ratio=0.01):
        super().__init__()
        self.adapter_attn_q = DiagonalLinear(block_size=dim, in_features=hidden_size, out_features=hidden_size, curvature=curvature_ratio)
        self.adapter_attn_v = DiagonalLinear(block_size=dim, in_features=hidden_size, out_features=hidden_size, curvature=curvature_ratio)
...
def set_Clip_Adapter(model, dim=8, s=0.1, curv=0.01):
    for _ in model.children():
        if type(_) == ResidualAttentionBlock:
            _.hyperbolic_attn = Adapter_qv(1024, dim=1, curvature_ratio=curv)
